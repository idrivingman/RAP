...
### 输入：
```
<Init_state>："实现一个python程序，用于根据用户提供的输入日期，返回当天是星期几。"</Init_state>
<Depth>：1</Depth>
<Curr_state>：
"实现一个python程序，用于根据用户提供的输入日期，返回当天是星期几"
</Curr_state>
```
### 输出：
```
1. 概念识别：[
   Init_state：要求实现根据输入的日期判定当天是星期几
...
action:让我们开始分步解决这个问题，首先第一步是导入处理日期必要的库datetime action:让我们开始分步解决这个问题，首先第一步是导入处理星期问题的python标准库模块calendar action:让我们开始分步解决这个问题，首先第一步是导入可以处理星期问题的pandas库
```
...
1. 评估代码实现的正确性和质量
2. 确保开发过程的渐进性，避免超前实现
3. 基于代码质量(60%)和目标贡献度(40%)两个维度给出最终的数值评分
...
# 评分标准
## 代码质量评估（权重50%）
- 评估对象：Next_state的代码实现质量
- 评分流程：从10分起，按照以下规则逐项扣分
- 最终分数：所有扣分项累加后与1分取最大值
...
### A. 合理推进区间（4-10分）
- 前提条件：当前Action只完成一个明确的子任务
- 评分标准：
  * 8-10分：完美完成当前Action定义的子任务，且为后续任务打下良好基础
  * 6-8分：很好地完成当前Action定义的子任务，实现规范完整
  * 4-6分：基本完成当前Action定义的子任务，有小的瑕疵或存在改进空间

### B. 不良推进区间（1-3分）
- 触发条件：
  * 一次性实现了超出当前Action范围的功能
  * 或未能完成当前Action定义的子任务
- 评分标准：
  * 3分：未能完全完成当前Action的要求
  * 2分：超前实现了下一步骤的部分功能或超前实现了多个步骤的功能
  * 1分：当前实现与Action要求不符或完全偏离了Action的要求或一次性实现了全部功能

# 最终得分计算流程
1. 分数计算
   - 代码质量得分 = 10 - 扣分总和（最低1分）
   - 目标贡献度得分 = 直接评分（1-10分）
   - 最终得分 = (代码质量得分 × 50% + 目标贡献度得分 × 50%) 四舍五入为整数
...
1. 评分过程：
   ...
   b. 目标贡献度评分：
      - 严格遵循当前Action的要求，只实现了节点类定义
      - 未提前实现反转逻辑
      - 为后续步骤提供了完善的基础
      得分：10分（合理推进区间）
   
   c. 最终计算：(10 × 50% + 10 × 50%) = 10分，四舍五入为10分

2. result:10

## 示例2：二分查找实现
### 输入：
<Init_state>："实现一个二分查找函数，在有序数组中查找目标值的索引"</Init_state>
<Action>："定义函数框架和基本的边界条件检查"</Action>
<Curr_state>：
"# TODO: 实现二分查找函数"
</Curr_state>
<Next_state>：
...
1. 评分过程：
   a. 代码质量评分：
      ...
      扣分项：
      - 右边界初始化错误（-8分）：
        * right设置为len(arr)会导致数组越界,正确的右边界应该是len(arr) - 1
        * 这个错误会影响整个算法的正确性
      得分：2分（重要错误）
   
   b. 目标贡献度评分：
      - 违背了当前Action的范围要求
      - 不仅实现了边界检查，还包含了完整的查找逻辑
      - 一次性完成了所有功能，没有体现渐进思维
      得分：2分（不良推进区间）
   
   c. 最终计算：(2 × 50% + 2 × 50%) = 2，四舍五入为2分
